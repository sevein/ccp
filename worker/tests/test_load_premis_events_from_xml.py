...
from worker.clientScripts import load_premis_events_from_xml
...
@pytest.fixture()
def xsd_path():
    return os.path.abspath(os.path.join(THIS_DIR, "../worker/assets/premis/premis.xsd"))
...
@mock.patch("worker.clientScripts.load_premis_events_from_xml.PREMISFile")
@mock.patch(
    "worker.clientScripts.load_premis_events_from_xml.get_premis_element_children_identifiers",
    return_value=set(),
)
...
def test_get_or_create_agents():
    ...
    with mock.patch(
        "worker.clientScripts.load_premis_events_from_xml.Agent", mock_agent_model
    ):
        agents = [
